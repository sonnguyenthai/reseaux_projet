...
int main(int argc, char *argv[])
{
  ...
  inet_pton(AF_INET6, hote,  &(sa.sin6_addr));

  /* On extrait l'addresse IP */
  sprintf(ip,"%s",inet_ntop(AF_INET6, &(sa.sin6_addr), ip, INET6_ADDRSTRLEN )) ;
//  sprintf(ip,"%s",inet_ntoa( ((struct sockaddr_in6*)resol->ai_addr)->sin6_addr) );
